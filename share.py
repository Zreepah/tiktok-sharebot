    ...
    try:
        ...
        try:
            if (req.json()["status_code"] == 0):
                ...
                TotalSendedShare += 1
                if DebugMode == True:
                    print(Colorate.Horizontal(Colors.red_to_purple, f"zreepah is a skid lmao {TotalSendedShare} "))
                else:
                    print(Colorate.Horizontal(Colors.red_to_purple, f"zreepah is a skid lmao {TotalSendedShare}"))
                    Title(f"zreepah is a skid lmao         Thread :{str(active_count()-1)} / Hit :{TotalSendedShare} / Fail :{TotalFailedReq}")
            else:
                pass
        except:
            TotalFailedReq += 1
...
if (__name__ == "__main__"):
    Clear()
    print(Colorate.Horizontal(Colors.red_to_purple, r"""

__________                                  .__     
\____    /______   ____   ____ ___________  |  |__  
  /     /\_  __ \_/ __ \_/ __ \\____ \__  \ |  |  \ 
 /     /_ |  | \/\  ___/\  ___/|  |_> > __ \|   Y  \
/_______ \|__|    \___  >\___  >   __(____  /___|  /
        \/            \/     \/|__|       \/     \/ 


            """))
    itemID       = Write.Input("Video Link > ", Colors.red_to_purple, interval=0.001)
    amount       = Write.Input("Amount (0=inf) > ", Colors.red_to_purple, interval=0.001)
    NThread      = Write.Input("Thread Amount > ", Colors.red_to_purple, interval=0.001)
